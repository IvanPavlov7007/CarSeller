...
/// 
/// Rules:
/// - Controlled actor (Current vehicle) is a vision center.
/// </summary>
public sealed class CityVision
{
    ...
    private void ApplyActorFromVehicleState(VehicleController.VehicleControlState state)
    {
        ...
        // Add new center.
        if (!nextActorEntity.TryAddAspect(new VisionCenterAspect(_defaultActorVision)))
        {
            Debug.LogWarning($"CityVision: Failed to add VisionCenterAspect on controlled actor {nextActorEntity.Subject}");
            _currentActorEntity = null;
            return;
        }
        _currentActorEntity = nextActorEntity;
    }
